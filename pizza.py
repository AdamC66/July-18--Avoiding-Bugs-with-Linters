    ...
    for pizza in current_order:
        ...
        while wait_for_input == True:
            ...
            if user_input in ["s", "m", "l", "xl"]:
                ...
                print(
                    "Your input was invalid or unrecognized, please enter S, M, L, or XL"
                )
    ...
    for pizza in current_order:
        for x in range(pizza.num_toppings):
            ...
            while wait_for_input == True:
                ...
                print(
                    'please note, cheese is automatically added to all orders, for no cheese, enter "no cheese"'
                )
                user_input = input().lower()
                if user_input == "list":
...
def display_order(current_order):
    ...
    for item in current_order:
        print(
            f"{item.name},",
            f"Size: {item.size.upper()},",
            f"{item.num_toppings} Toppings",
        )
...
for i in range(len(current_order)):
    current_order[i].num_toppings = get_input(
        f"How many toppings would you like on {current_order[i].name}"
    )
